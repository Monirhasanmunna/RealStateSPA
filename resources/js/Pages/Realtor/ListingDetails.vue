...
<template>
    <Head title="Your Listing Details" />
    <MainLayout>
        <BackButtonComponent title="Listing Show" :href="route('realtor.listing')" class="flex btn-green" />
        <div>
            <div class="flex flex-col-reverse md:grid md:grid-cols-12 gap-4">
                <div class="col-span-7 flex items-center w-full box-border p-3">
                    <div v-if="listing.images.length" class="grid grid-cols-2 gap-1">
                        <img class="h-full" v-for="image in listing.images" :key="image.id" :src="image.src" alt="">
                    </div>
